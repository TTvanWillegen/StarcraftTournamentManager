...
/**
 * A Poule object contains multiple {@link Match}es aswell as an {@link Order} in which to
...
public class Poule {
     ...
     *                                  objects.Order} defined in the Poule
     */
     ...
     * removed.
     *
     * @param matchToRemove The {@link Match} to remove from the {@link objects.Poule}
    ...
    private List<Match> returnMatchesInLine() {
        return matchList.stream().filter(
            match -> match.getMatchState() == MatchState.IN_LINE).collect(Collectors.toList());
    }
